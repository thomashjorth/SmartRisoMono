...
namespace DataModel.ConfigurationModel.Factory
{
	...
	public class MAggregationVisualizationFactory: AbstractMultiAggregationVisualizationFactory
	{
		...
		public override VisualizationConfig CreateBar(List<string> hosts, List<int> ports, RealtimeInterface deviceInterface, RealtimeData data,int updateInterval,string titleHeading,int valueMin,int valueMax,string unit)
		{
			Parameters = "?hosts=";
			for(int i = 0; i<hosts.Count; i++) {
				Parameters += hosts[i] + ":" + ports[i] +";";
			...
			Parameters = Parameters.Replace(";?","");
			var p = "GetUnits/"+Parameters+ "&wsInterface="+deviceInterface+"&resource=get"+data;
			return new BarConfig (Host, Port, Device, p, updateInterval, titleHeading, valueMin, valueMax, unit);
		}
		 
		public override VisualizationConfig CreatePie(List<string> hosts, List<int> ports, RealtimeInterface deviceInterface, RealtimeData data,int updateInterval,string titleHeading)
		{
			Parameters = "?hosts=";
			for(int i = 0; i<hosts.Count; i++) {
				Parameters += hosts[i] + ":" + ports[i] +";";
			...
			Parameters = Parameters.Replace(";?","");
			var p = "GetUnits/"+Parameters+ "&wsInterface="+deviceInterface+"&resource=get"+data;
			return new PieConfig (Host, Port, Device, p, updateInterval, titleHeading);
		}
